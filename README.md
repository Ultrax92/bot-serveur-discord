...
|---|---|---|
| 🔨 Modération | ✅ Fait | warn, mute/unmute (timeout), kick, ban/tempban, unban, clear, clean-channel (purge complète en arrière-plan, owner), sanctions, lock, hide, role — les commandes à variantes utilisent une option "action" à choix plutôt que des sous-commandes |
...
| 🎫 Tickets | ✅ Fait | configuré via `/setup` → 🎫 : types créés/modifiés/supprimés librement (catégorie, rôles mentionnés/accès, message d'ouverture chacun), panneau à sélecteur publié, salons numérotés (0001…), claim/close (boutons + `/ticket action:[ajouter/retirer/claim/fermer] [membre] [ticket par ID/lien, vide = ticket courant]`), max par personne, rôle requis, fermeture au départ du membre, transcript complet .txt en MP + logs-tickets, automod inactif dans les tickets, notation des avis clients (`/setup` → 🎫 → ⭐) : MP 1-5⭐ + commentaire à la fermeture, validation staff avant publication dans le salon feedback (en MP au owner si aucun salon de validation), avis 5⭐ générique auto à J+7 sans réponse ou si le membre est parti, rôle client à la publication |
