...
module.exports = {
  ...
  async execute(message) {
    ...
    try {
      trackTicketActivity(message);
    } catch (error) {
      console.error('Erreur suivi activité ticket :', error);
    }
    ...
    const reviewImageConsumed = await handlePendingReviewImage(message).catch((error) => {
      console.error('Erreur image avis :', error);
      return false;
    });
    ...
    const backupConsumed = await handlePendingBackupFile(message).catch((error) => {
    ...
    const consumed = await handlePendingImage(message).catch((error) => {
      console.error('Erreur upload image :', error);
      return false;
    });
    if (consumed) return;

    const blocked = await handleMessage(message).catch((error) => {
