...
const insertStmt = db.prepare(
  'INSERT OR REPLACE INTO tempvoc_channels (channel_id, guild_id, owner_id) VALUES (?, ?, ?)',
);
...
async function applyGeneratorPermissions(guild) {
  ...
  const ok = await channel.permissionOverwrites
    .set(overwrites, 'Configuration des vocaux temporaires')
    .then(() => true)
    .catch(() => false);
  return ok;
}
...
async function handleVoiceState(oldState, newState) {
  ...
  if (
    newState.channelId &&
    newState.channelId === config.generatorChannel &&
    newState.member &&
    !newState.member.user.bot
  ) {
    const member = newState.member;
    const name = config.nameTemplate.replaceAll('{pseudo}', member.displayName ?? member.user.username).slice(0, 100);
...
async function handleTempvocComponent(interaction) {
  ...
  if (!row) {
    return interaction.reply({
      content: "Ce salon n'est pas (ou plus) un salon vocal temporaire.",
      flags: MessageFlags.Ephemeral,
    });
  }
  if (interaction.user.id !== row.owner_id && !isBotAdmin(interaction)) {
    return interaction.reply({
      content: `Seul <@${row.owner_id}> (le propriétaire du salon) peut le gérer.`,
      flags: MessageFlags.Ephemeral,
    });
  }
    ...
    const modal = new ModalBuilder()
      ...
      .addComponents(
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            .setCustomId('limit')
            .setLabel('Nombre de places (0 = illimité, max 99)')
  ...
  if (action === 'modal') {
    ...
    if (kind === 'rename') {
      ...
      return interaction.reply({
        content: `✏️ Salon renommé en **${name}** (peut prendre quelques instants, limite Discord).`,
        flags: MessageFlags.Ephemeral,
      });
    }
    if (kind === 'limit') {
      ...
      if (!Number.isInteger(limit) || limit < 0 || limit > 99) {
        return interaction.reply({
          content: '❌ Valeur invalide : entre 0 (illimité) et 99.',
          flags: MessageFlags.Ephemeral,
        });
      }
