...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (durationInput) {
      ...
      if (!duration) {
        return interaction.reply({
          embeds: [errorEmbed(interaction, 'Durée invalide. Exemples : `12h`, `7j`, `1j12h`.')],
    ...
    if (getSettings(interaction.guildId).moderationConfig.dmOnSanction) {
      await user
        .send(
          `🔨 Tu as été banni de **${interaction.guild.name}**${durationText ? durationText.replaceAll('**', '') : ''} : ${reason}`,
        )
        .catch(() => {});
    }
    ...
    await logModAction(interaction, {
      ...
      target: user,
      reason,
      duration: expiresAt ? durationText.replaceAll('*', '').replace(' pour ', '') : null,
    });
    return interaction.reply({
      embeds: [successEmbed(interaction, `🔨 **${user.tag}** a été banni${durationText}.\n**Raison :** ${reason}`)],
    });
  },
};
